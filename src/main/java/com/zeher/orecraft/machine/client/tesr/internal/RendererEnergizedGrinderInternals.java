...
public class RendererEnergizedGrinderInternals extends TileEntitySpecialRenderer<TileEntityEnergizedGrinder>{

	@SideOnly(Side.CLIENT)
	private ModelBlockGrinderInternals model;
	
	public RendererEnergizedGrinderInternals() {
		this.model = new ModelBlockGrinderInternals();
	}
	
	@Override
	public void render(TileEntityEnergizedGrinder tileentity, double x, double y, double z, float partialTicks, int destroyStage, float a) {
		if (tileentity == null) {
			return;
		}
		
		IBlockState block = tileentity.getWorld().getBlockState(tileentity.getPos());
		BlockEnergizedGrinder block_ = (BlockEnergizedGrinder) block.getBlock();
		...
		if (tileentity.canGrindOne() && tileentity.getCookTime(0) > 0 && tileentity.hasStored() || tileentity.canGrindTwo() && tileentity.getCookTime(1) > 0 && tileentity.hasStored()) {
