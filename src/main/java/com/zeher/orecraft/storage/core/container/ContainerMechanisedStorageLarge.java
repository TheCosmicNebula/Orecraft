...
public class ContainerMechanisedStorageLarge extends Container {
	...
	public ContainerMechanisedStorageLarge(InventoryPlayer invPlayer, IInventory tile) {
		...
		for (int y = 0; y < 7; y++) {
			for (int x = 0; x < 13; x++) {
				this.addSlotToContainer(new Slot(tile, x + y * 13, 8 + x * 18, 16 + y * 18));
			}
		}
		
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 9; x++) {
				this.addSlotToContainer(new Slot(invPlayer, 9 + x + y * 9, 44 + x * 18, 156 + y * 18));
			}
		}
		
		for (int x = 0; x < 9; x++) {
			this.addSlotToContainer(new Slot(invPlayer, x, 44 + x * 18, 214));
		}
