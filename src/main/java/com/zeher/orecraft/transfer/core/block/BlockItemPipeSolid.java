...
public class BlockItemPipeSolid extends BlockItemPipe {
	...
	public BlockItemPipeSolid(String name, Material material, String tool, int harvest, int hardness, int resistance,
			CreativeTabs tab) {
	...
	public TileEntity createNewTileEntity(World arg0, int arg1) {
		return new TileEntityItemPipeSolid();
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		TileEntityItemPipeSolid tile = (TileEntityItemPipeSolid) world.getTileEntity(pos);
		
		if ((tile instanceof TileEntityItemPipeSolid) && tile != null) {
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		if (tile != null && tile instanceof TileEntityItemPipeSolid) {
			northInt = ((TileEntityItemPipeSolid) tile).getSide(EnumFacing.NORTH);
			eastInt = ((TileEntityItemPipeSolid) tile).getSide(EnumFacing.EAST);
			southInt = ((TileEntityItemPipeSolid) tile).getSide(EnumFacing.SOUTH);
			westInt = ((TileEntityItemPipeSolid) tile).getSide(EnumFacing.WEST);
			downInt = ((TileEntityItemPipeSolid) tile).getSide(EnumFacing.DOWN);
			upInt = ((TileEntityItemPipeSolid) tile).getSide(EnumFacing.UP);
		} else {
