...
public class ContainerPoweredCompressor extends Container {
	...
	public ContainerPoweredCompressor(InventoryPlayer invPlayer, IInventory tile) {
		...
		/**@OutputSlot*/
		this.addSlotToContainer(new SlotCompressor(invPlayer.player, tile, 2, 104, 60));
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index == 2) {
				...
				if (!CompressorRecipes.getInstance().getCompressingResult(itemstack1).isEmpty()) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false)) {
						return ItemStack.EMPTY;
					}
				} else if (TileEntityPoweredCompressor.isItemPower(itemstack1)) {
