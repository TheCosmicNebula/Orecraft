...
public class RendererEnergizedFurnaceInternals extends TileEntitySpecialRenderer<TileEntityEnergizedFurnace> {
	
	@SideOnly(Side.CLIENT)
	private ModelBlockFurnaceInternals model;
	
	public RendererEnergizedFurnaceInternals() {
		this.model = new ModelBlockFurnaceInternals();
	}
	
	@Override
	public void render(TileEntityEnergizedFurnace tileentity, double x, double y, double z, float partialTicks, int destroyStage, float a) {
		if (tileentity == null) {
			return;
		}
		
		IBlockState state = tileentity.getWorld().getBlockState(tileentity.getPos());
		BlockEnergizedFurnace block = (BlockEnergizedFurnace) state.getBlock();
		...
		if (tileentity.canSmeltOne() && tileentity.getCookTime(0) > 0 && tileentity.hasStored() || tileentity.canSmeltTwo() && tileentity.getCookTime(1) > 0 && tileentity.hasStored()) {
