...
public class BlockPoweredFluidCrafter extends BlockFluidMachine implements ITileEntityProvider {
	...
	public BlockPoweredFluidCrafter(String name, Material material, String tool, int harvest, int hardness, int resistance, CreativeTabs tab) {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(BlockHandler.REGISTRATION.FLUIDMACHINE.BLOCK_POWERED_FLUIDCRAFTER);
	}
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    	...
    	if(!(worldIn.isRemote) && !(playerIn.isSneaking()) && !(ModUtil.isHoldingHammer(playerIn)) && stack.isEmpty()){
			FMLNetworkHandler.openGui(playerIn, Orecraft.INSTANCE, 43, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
			...
			if (liquid != null) {
				...
				if (amount == liquid.amount) {
					if(liquid.getFluid().getTemperature() < 800){	
						...
						if (tank.getTank().getFluid().getFluid().getBlock() != null) {
	...
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
		return new ItemStack(BlockHandler.REGISTRATION.FLUIDMACHINE.BLOCK_POWERED_FLUIDCRAFTER);
	}
