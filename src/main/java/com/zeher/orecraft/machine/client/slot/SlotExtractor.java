...
public class SlotExtractor extends Slot {
	...
	public SlotExtractor(EntityPlayer par1EntityPlayer, IInventory par2IInventory, int par3, int par4, int par5) {
	...
	protected void onCrafting(ItemStack stack) {
		...
		if (!this.player.world.isRemote) {
			int i = this.amount;
			float f = ExtractorRecipes.getInstance().getExtractingExperience(stack);
			if (f == 0.0F) {
				i = 0;
			} else if (f < 1.0F) {
