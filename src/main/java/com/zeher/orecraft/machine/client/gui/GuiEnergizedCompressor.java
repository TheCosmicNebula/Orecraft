...
public class GuiEnergizedCompressor extends GuiContainer {
	...
	public GuiEnergizedCompressor(InventoryPlayer par1InventoryPlayer, TileEntityEnergizedCompressor par2TileEntityCompressor) {
	...
	protected void drawGuiContainerForegroundLayer(int mouse_x, int mouse_y) {
		...
		if (ModGuiUtil.IS_HOVERING.isHoveringPowerSmall(mouse_x, mouse_y, screen_x + 68, screen_y + 16)) {
			if (this.INVENTORY.getCookTime(0) > 0 || this.INVENTORY.getCookTime(1) > 0 || this.INVENTORY.canCompressOne() || this.INVENTORY.canCompressTwo() && this.INVENTORY.hasStored()) {
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int mouse_x, int mouse_y) {
		...
		this.mc.getTextureManager().bindTexture(OrecraftReference.GUI.RESOURCE.GUI_COMPRESSOR_TWO);
