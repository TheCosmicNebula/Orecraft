...
import com.zeher.orecraft.production.client.tesr.model.ModelBlockHeatedFluidGenerator;
...
public class RendererPoweredHeatedFluidGenerator extends TileEntitySpecialRenderer<TileEntityPoweredHeatedFluidGenerator> {

	@SideOnly(Side.CLIENT)
	private ModelBlockHeatedFluidGenerator model;

	@SideOnly(Side.CLIENT)
	public RendererPoweredHeatedFluidGenerator() {
		this.model = new ModelBlockHeatedFluidGenerator();
	}

	private static final ResourceLocation texture_location = new ResourceLocation(OrecraftReference.RESOURCE.TESR + "production/powered/generator/heatedfluid/heated_fluid.png");
	
	@Override
	public void render(TileEntityPoweredHeatedFluidGenerator tileentity, double x, double y, double z, float partialTicks, int destroyStage, float a) {
		IBlockState state = tileentity.getWorld().getBlockState(tileentity.getPos());
		BlockPoweredHeatedFluidGenerator block = (BlockPoweredHeatedFluidGenerator) state.getBlock();
