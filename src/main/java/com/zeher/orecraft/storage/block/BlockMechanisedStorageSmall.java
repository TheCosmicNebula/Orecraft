...
public class BlockMechanisedStorageSmall extends TRZBlockItemStorage {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (!(worldIn.isRemote) && !(playerIn.isSneaking()) && !(TRZUtil.isHoldingHammer(playerIn))) {
			FMLNetworkHandler.openGui(playerIn, OreCraft.instance, 56, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
