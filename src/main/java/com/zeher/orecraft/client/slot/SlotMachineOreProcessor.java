...
public class SlotMachineOreProcessor extends Slot {
	private EntityPlayer thePlayer;
	private int field_75228_b;

	public SlotMachineOreProcessor(EntityPlayer par1EntityPlayer, IInventory par2IInventory, int par3, int par4,
			int par5) {
	...
	protected void onCrafting(ItemStack stack) {
		...
		if (!this.thePlayer.world.isRemote) {
			...
			float f = MachineProcessorCleaningRecipes.instance().getCleaningExperience(stack);
			if (f == 0.0F) {
				i = 0;
			} else if (f < 1.0F) {
