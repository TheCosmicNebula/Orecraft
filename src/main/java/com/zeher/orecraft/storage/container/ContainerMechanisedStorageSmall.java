...
public class ContainerMechanisedStorageSmall extends Container {
	...
	public ContainerMechanisedStorageSmall(InventoryPlayer invPlayer, IInventory tile) {
		...
		for (int x = 0; x < 9; x++) {
			addSlotToContainer(new Slot(invPlayer, x, 17 + x * 18, 178));
		}
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 9; x++) {
				addSlotToContainer(new Slot(invPlayer, 9 + x + y * 9, 17 + x * 18, 120 + y * 18));
			}
		}
		
		for(int y = 0; y < 5; y++){
			for(int x = 0; x < 10; x++){
				addSlotToContainer(new Slot(tile, x + y * 10, 8 + x * 18, 16 + y * 18));
			}
		}
	}
