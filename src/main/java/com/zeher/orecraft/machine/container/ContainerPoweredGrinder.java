...
import com.zeher.orecraft.client.slot.SlotMachineGrinder;
...
public class ContainerPoweredGrinder extends Container {
	...
	public ContainerPoweredGrinder(InventoryPlayer invPlayer, IInventory tile) {
		this.inventory = tile;
		for (int x = 0; x < 9; x++) {
		...
		addSlotToContainer(new SlotMachineGrinder(invPlayer.player, tile, 2, 116, 35));
	...
	public void detectAndSendChanges() {
		...
		for (int i = 0; i < this.listeners.size(); i++) {
			...
			if (this.grinderBurnTime != this.inventory.getField(0)) {
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index == 2) {
				...
				} else if (TileEntityPoweredGrinder.isItemPower(itemstack1)) {
