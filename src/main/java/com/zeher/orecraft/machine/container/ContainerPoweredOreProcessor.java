...
import com.zeher.orecraft.client.slot.SlotMachineOreProcessor;
...
public class ContainerPoweredOreProcessor extends Container {
	...
	public ContainerPoweredOreProcessor(InventoryPlayer invPlayer, IInventory tile) {
		this.inventory = tile;
		for (int x = 0; x < 9; x++) {
			addSlotToContainer(new Slot(invPlayer, x, 15 + x * 18, 142));
		}
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 9; x++) {
				addSlotToContainer(new Slot(invPlayer, 9 + x + y * 9, 15 + x * 18, 84 + y * 18));
			}
		}
		
		addSlotToContainer(new Slot(tile, 0, 54, 10));
		addSlotToContainer(new Slot(tile, 1, 54, 50));
		addSlotToContainer(new SlotMachineOreProcessor(invPlayer.player, tile, 2, 91, 19));

		addSlotToContainer(new TRZSlotMachineUpgrade(tile, 3, 8, 8, ItemHandler.upgrade_speed));
		addSlotToContainer(new TRZSlotMachineUpgrade(tile, 4, 8, 30, ItemHandler.upgrade_capacity));
		addSlotToContainer(new TRZSlotMachineUpgrade(tile, 5, 8, 52, ItemHandler.upgrade_charge));
		
		addSlotToContainer(new TRZSlotMachineUpgrade(tile, 6, 166, 8, ItemHandler.upgrade_fluid_speed));
		addSlotToContainer(new TRZSlotMachineUpgrade(tile, 7, 166, 30, ItemHandler.upgrade_fluid_capacity));
		addSlotToContainer(new TRZSlotMachineUpgrade(tile, 8, 166, 52, ItemHandler.upgrade_fluid_efficiency));
		
		addSlotToContainer(new TRZSlotBucket(tile, 9, 122, 8));
		addSlotToContainer(new TRZSlotNoAccess(tile, 10, 122, 52, 16));
	}
