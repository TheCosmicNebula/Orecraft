...
public class TileEntityHeatedFluidGeneratorBasic extends TRZTileEntityFluidProducer implements ITickable, ISidedInventory, TRZIProducer, IFluidHandler {
		...
		if (!this.world.isRemote) {
			...
			if (canBurn() && this.stored < this.capacity) {
				...
				if (this.generate_burn_time == generation_time - 1) {
					if(this.tank.getFluid().getFluid().getTemperature() > 1000){
			...
			if (flag == this.isBurning()) {
				flag1 = true;
				BlockHeatedFluidGeneratorBasic.setState(this.isBurning(), this.world, this.pos);
			}
