...
public class RendererPoweredPeltierGenerator extends TileEntitySpecialRenderer<TileEntityPoweredPeltierGenerator> {

	@SideOnly(Side.CLIENT)
	private ModelBlockPeltierGenerator model;

	@SideOnly(Side.CLIENT)
	public RendererPoweredPeltierGenerator() {
		this.model = new ModelBlockPeltierGenerator();
	}

	private static final ResourceLocation texture_location = new ResourceLocation(OrecraftReference.RESOURCE.TESR + "production/powered/generator/peltier/peltier.png");
	
	@Override
	public void render(TileEntityPoweredPeltierGenerator tileentity, double x, double y, double z, float partialTicks, int destroyStage, float a) {
		IBlockState state = tileentity.getWorld().getBlockState(tileentity.getPos());
		BlockPoweredPeltierGenerator block = (BlockPoweredPeltierGenerator) state.getBlock();
