...
public class GuiPoweredFurnace extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int mouse_x, int mouse_y) {
		...
		if (ModGuiUtil.IS_HOVERING.isHoveringPowerSmall(mouse_x, mouse_y, screen_coords[0] + 76, screen_coords[1] + 17)) {
			if (this.INVENTORY.getCookTime(0) > 0 || this.INVENTORY.canSmelt() && this.INVENTORY.hasStored()) {
	...
	protected void drawGuiContainerBackgroundLayer(float ticks, int mouse_x, int mouse_y) {
		...
		ModGuiUtil.DRAW.drawBackground(this, screen_coords, OrecraftReference.GUI.RESOURCE.GUI_FURNACE);
