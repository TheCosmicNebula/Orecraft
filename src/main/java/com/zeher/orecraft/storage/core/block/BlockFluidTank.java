...
public class BlockFluidTank extends ModBlockFluidTank implements ITileEntityProvider {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if ((ModUtil.isHoldingHammer(player)) && (player.isSneaking()) && !(world.isRemote)) {
			...
			player.swingArm(hand);
			ModFluidUtil.dropStackInWorld(world, pos, StorageUtil.getTankStackFromTile(tank, true, this));
		...
		if (stack.isEmpty()) {
			if (world.isRemote) {
				...
				if (tank.getCurrentStoredAmount() > 0) {
					...
					player.sendMessage(new TextComponentString("Fluid tank: [" + fluidName + "] (" + tank.getCurrentStoredAmount() + " / " + tank.getCapacity() + " mB)"));
				} else {
					player.sendMessage(new TextComponentString("Fluid tank: [Empty] (" + "0" +  " / " + tank.getCapacity() + " mB)"));
				}
			}
		}
			...
			if (liquid != null) {
				...
				if (amount == liquid.amount) {
					...
					if (!player.capabilities.isCreativeMode)
						player.inventory.setInventorySlotContents(player.inventory.currentItem, ModFluidUtil.useItemSafely(stack));
	...
	public void onBlockExploded(World world, BlockPos pos, Explosion explosion) {
		ModFluidUtil.dropStackInWorld(world, pos, StorageUtil.getTankStackFromTile((TileEntityFluidTank) world.getTileEntity(pos), true, this));
