...
public class TileEntityPoweredPeltierGenerator extends TileEntityLockable implements ITickable, ISidedInventory, IFluidProducer, IFluidHandler {
	...
	public int generation_rate = Reference.VALUE.PRODUCER.poweredGenerationRate(0);
	...
	public String getName() {
		return this.hasCustomName() ? this.custom_name : OrecraftReference.GUI_NAME.PRODUCER.POWERED.PELTIER;
	}
	...
	public void update() {
		...
		if (this.canProduce() && this.stored < this.capacity) {
			...
			if (this.tank.getFluidAmount() <= this.cook_speed) {
				this.tank.drain(2, true);
			} else {
				this.tank.drain(1000 / (this.cook_speed), true);
			}
			
			if (this.tank_two.getFluidAmount() <= this.cook_speed / 2) {
				this.tank_two.drain(2, true);
			} else {
				this.tank_two.drain(1000 / (this.cook_speed / 2) , true);
			}
	...
	public boolean canProduce() {
		if(this.isFluidEmpty() || this.isFluidEmptyTwo()){
			return false;
		} else {
			if (this.tank.getFluid().getFluid().getTemperature() > 1000 && !this.isFluidEmpty()) {
				if (this.tank_two.getFluid().getFluid().getTemperature() < 1000 && !this.isFluidEmptyTwo()) {
					return true;
				}	
			}
		}
