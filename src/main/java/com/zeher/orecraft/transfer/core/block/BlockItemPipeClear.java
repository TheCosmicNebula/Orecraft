...
public class BlockItemPipeClear extends BlockItemPipe {
	...
	public BlockItemPipeClear(String name, Material material, String tool, int harvest, int hardness, int resistance,
			CreativeTabs tab) {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if ((tile instanceof TileEntityItemPipeClear) && tile != null) {
			...
			} else if ((ModUtil.isHoldingHammer(player)) && (!player.isSneaking())) {
				player.swingArm(EnumHand.MAIN_HAND);
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		if (tile != null && tile instanceof TileEntityItemPipeClear) {
			northInt = ((TileEntityItemPipeClear) tile).getSide(EnumFacing.NORTH);
			eastInt = ((TileEntityItemPipeClear) tile).getSide(EnumFacing.EAST);
			southInt = ((TileEntityItemPipeClear) tile).getSide(EnumFacing.SOUTH);
			westInt = ((TileEntityItemPipeClear) tile).getSide(EnumFacing.WEST);
			downInt = ((TileEntityItemPipeClear) tile).getSide(EnumFacing.DOWN);
			upInt = ((TileEntityItemPipeClear) tile).getSide(EnumFacing.UP);
		} else {
		...
		return state.withProperty(NORTH, northBool).withProperty(EAST, eastBool).withProperty(SOUTH, southBool).withProperty(WEST, westBool).withProperty(UP, upBool).withProperty(DOWN, downBool);
	}
	...
	private boolean canCableConnectTo(IBlockAccess world, BlockPos pos, EnumFacing facing) {
		...
		if (tile != null) {
			if (tile instanceof IItemPipe) {
				
				if (((IItemPipe) tile).getSide(facing) == 1) {
					return false;
				} else {
					return true;
				}
	...
	public boolean canConnectTo(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		...
		return Connection.PIPE.BLOCK.getItemConnectionsAccess(worldIn, pos, side);
	}
