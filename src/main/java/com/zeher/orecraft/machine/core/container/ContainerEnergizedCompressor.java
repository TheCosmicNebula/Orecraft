...
import com.zeher.orecraft.machine.client.slot.SlotCompressor;
...
public class ContainerEnergizedCompressor extends Container {
	...
	public ContainerEnergizedCompressor(InventoryPlayer invPlayer, IInventory tile) {
		this.inventory = tile;
		...
		/**@Outputs*/
		this.addSlotToContainer(new SlotCompressor(invPlayer.player, tile, output_slot2, 112, 60));
		this.addSlotToContainer(new SlotCompressor(invPlayer.player, tile, output_slot, 96, 60));
	...
	public void detectAndSendChanges() {
		...
		for (int i = 0; i < this.listeners.size(); i++) {
			...
			if (this.power != this.inventory.getField(1)) {
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			// itemstack is in player inventory, try to place in appropriate furnace slot
				...
				if (!CompressorRecipes.getInstance().getCompressingResult(itemstack1).isEmpty() && !(itemstack1.getItem() instanceof ItemStorage)) {
				...
				else if (TileEntityEnergizedCompressor.isItemPower(itemstack1)) {
