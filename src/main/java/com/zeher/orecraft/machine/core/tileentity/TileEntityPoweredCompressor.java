...
public class TileEntityPoweredCompressor extends TileEntityLockable implements ITickable, ISidedInventory, IMachine {
	
	private static final int[] SLOTS_TOP = new int[] { 0 };
	private static final int[] SLOTS_BOTTOM = new int[] { 2, 1 };
	private static final int[] SLOTS_SIDES = new int[] { 1 };
	
	private NonNullList<ItemStack> compressor_stacks = NonNullList.<ItemStack>withSize(6, ItemStack.EMPTY);
	...
	public String getName() {
		return this.hasCustomName() ? this.custom_name : OrecraftReference.GUI_NAME.MACHINE.POWERED.COMPRESSOR;
	}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		compound.setInteger("CookTime", this.cook_time);
	...
	public void update() {
		if (this.hasStored() && this.canCompress()) {
		...
		if (this.hasStored() && this.canCompress()) {
			...
			if (this.sound_timer > 45) {
				if (this.world.isRemote) {
					this.world.playSound(this.pos.getX() + 0.5D, this.pos.getY(), this.pos.getZ() + 0.5D, SoundHandler.MACHINE.COMPRESSOR, SoundCategory.BLOCKS, 0.15F, 1.0F, false);
				}
				this.sound_timer = 0;
			}
		} else {
		...
		int i = this.compressor_stacks.get(3).getCount();
		this.cook_speed = Reference.VALUE.MACHINE.poweredProcessSpeed(i);
		
		int i_one = this.compressor_stacks.get(4).getCount();
		this.capacity = Reference.VALUE.MACHINE.poweredStored(i_one);
		
		if (!this.world.isRemote) {
			if ((isItemPower(this.compressor_stacks.get(1))) && (this.stored <= this.capacity - ItemPowerValues.getItemPower(this.compressor_stacks.get(1).getItem()))) {
	...
	public boolean canCompress() {
		if (this.compressor_stacks.get(0).isEmpty()) {
			...
			ItemStack itemstack = CompressorRecipes.getInstance().getCompressingResult((ItemStack) this.compressor_stacks.get(0));

			if (itemstack.isEmpty()) {
				return false;
			} else {
				ItemStack itemstack1 = this.compressor_stacks.get(2);
	...
	public void compressItem() {
		if (this.canCompress()) {
			ItemStack itemstack = this.compressor_stacks.get(0);
			ItemStack itemstack1 = CompressorRecipes.getInstance().getCompressingResult(itemstack);
			ItemStack itemstack2 = this.compressor_stacks.get(2);

			if (itemstack2.isEmpty()) {
				this.compressor_stacks.set(2, itemstack1.copy());
			} else if (itemstack2.getItem() == itemstack1.getItem()) {
				itemstack2.grow(itemstack1.getCount());
			}

			if (itemstack.getItem() == Item.getItemFromBlock(Blocks.SPONGE) && itemstack.getMetadata() == 1 && !((ItemStack) this.compressor_stacks.get(1)).isEmpty() && ((ItemStack) this.compressor_stacks.get(1)).getItem() == Items.BUCKET) {
				this.compressor_stacks.set(1, new ItemStack(Items.WATER_BUCKET));
			}
