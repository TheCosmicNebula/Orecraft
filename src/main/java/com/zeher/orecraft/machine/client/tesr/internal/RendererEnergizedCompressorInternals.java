...
public class RendererEnergizedCompressorInternals extends TileEntitySpecialRenderer<TileEntityEnergizedCompressor> {

	@SideOnly(Side.CLIENT)
	private ModelBlockCompressorInternals model;
	
	public RendererEnergizedCompressorInternals() {
		this.model = new ModelBlockCompressorInternals();
	}
	
	@Override
	public void render(TileEntityEnergizedCompressor tileentity, double x, double y, double z, float partialTicks, int destroyStage, float a) {
		if (tileentity == null) {
			return;
		}
		
		IBlockState state = tileentity.getWorld().getBlockState(tileentity.getPos());
		BlockEnergizedCompressor block = (BlockEnergizedCompressor) state.getBlock();
		...
		if (tileentity.canCompressOne() && tileentity.getCookTime(0) > 0 && tileentity.hasStored() || tileentity.canCompressTwo() && tileentity.getCookTime(1) > 0 && tileentity.hasStored()) {
