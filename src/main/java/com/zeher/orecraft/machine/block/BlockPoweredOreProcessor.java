...
public class BlockPoweredOreProcessor extends TRZBlockFluidProducer implements ITileEntityProvider{
	...
	public BlockPoweredOreProcessor(String name, Material material, String tool, int harvest, int hardness, int resistance, CreativeTabs tab, boolean isBurning) {
	...
	public IBlockState getExtendedState(IBlockState state, IBlockAccess world, BlockPos pos) {
		...
		TileEntityPoweredOreProcessor tankEntity = (TileEntityPoweredOreProcessor) world.getTileEntity(pos);
		BlockPoweredOreProcessor block = (BlockPoweredOreProcessor) world.getBlockState(pos).getBlock();
    ...
    {
        return Item.getItemFromBlock(BlockHandler.block_powered_oreprocessor);
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
    	...
    	if(!(worldIn.isRemote) && !(playerIn.isSneaking()) && !(TRZUtil.isHoldingHammer(playerIn)) && stack.isEmpty()){
			FMLNetworkHandler.openGui(playerIn, OreCraft.instance, 61, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
    	
    	if (stack != null) {
			FluidStack liquid = FluidUtil.getFluidContained(stack);
			TileEntityPoweredOreProcessor tank = (TileEntityPoweredOreProcessor) worldIn.getTileEntity(pos);
			if (liquid != null) {
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos)
    {
        ...
        if (active)
        {
            worldIn.setBlockState(pos, BlockHandler.block_powered_oreprocessor_on.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
            worldIn.setBlockState(pos, BlockHandler.block_powered_oreprocessor_on.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        }
        else
        {
            worldIn.setBlockState(pos, BlockHandler.block_powered_oreprocessor.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
            worldIn.setBlockState(pos, BlockHandler.block_powered_oreprocessor.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        }
    ...
    {
        return new TileEntityPoweredOreProcessor();
    }
    ...
    {
        return new ItemStack(BlockHandler.block_powered_oreprocessor);
    }
