...
public class BakedFluidPipeClearFluidModel implements IBakedModel
{
	...
	public BakedFluidPipeClearFluidModel(Fluid fluid, double side_connect)
	{
        ...
        if(side_connect == 0) { BakedUtils.renderBase(fluid, quad_builder, texture, format, faceQuads); }
        if(side_connect == 1) { BakedUtils.renderDown(fluid, quad_builder, texture, format, faceQuads); }
        if(side_connect == 2) { BakedUtils.renderUp(fluid, quad_builder, texture, format, faceQuads); }
        if(side_connect == 3) { BakedUtils.renderUpDown(fluid, quad_builder, texture, format, faceQuads); }
        if(side_connect == 4) { BakedUtils.renderNorth(fluid, quad_builder, texture, format, faceQuads); }
        if(side_connect == 5) { BakedUtils.renderNorthDown(fluid, quad_builder, quad_builder_two, texture, format, faceQuads); }
