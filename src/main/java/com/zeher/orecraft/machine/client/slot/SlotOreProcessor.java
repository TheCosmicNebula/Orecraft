...
public class SlotOreProcessor extends Slot {
	private EntityPlayer player;
	private int amount;

	public SlotOreProcessor(EntityPlayer par1EntityPlayer, IInventory par2IInventory, int par3, int par4,
			int par5) {
	...
	protected void onCrafting(ItemStack stack) {
		...
		if (!this.player.world.isRemote) {
			int i = this.amount;
			float f = OreProcessorCleaningRecipes.getInstance().getCleaningExperience(stack);
			if (f == 0.0F) {
				i = 0;
			} else if (f < 1.0F) {
