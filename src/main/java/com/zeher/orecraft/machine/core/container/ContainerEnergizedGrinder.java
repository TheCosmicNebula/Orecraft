...
import com.zeher.orecraft.machine.client.slot.SlotGrinder;
...
public class ContainerEnergizedGrinder extends Container {
	...
	public ContainerEnergizedGrinder(InventoryPlayer invPlayer, IInventory tile) {
		...
		/**@Outputs*/
		this.addSlotToContainer(new SlotGrinder(invPlayer.player, tile, output_slot2, 112, 60));
		this.addSlotToContainer(new SlotGrinder(invPlayer.player, tile, output_slot, 96, 60));
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			// itemstack is in player inventory, try to place in appropriate furnace slot
				...
				if (!GrinderRecipes.getInstance().getGrindingResult(itemstack1).isEmpty() && !(itemstack1.getItem() instanceof ItemStorage)) {
				...
				else if (TileEntityEnergizedGrinder.isItemPower(itemstack1)) {
