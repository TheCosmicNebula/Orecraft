		...
		public IMessage onMessage(final PacketFluidCrafter message, final MessageContext ctx) {
			...
			if(tile_two != null){
				if(tile_two instanceof TileEntityPoweredFluidCrafter){
					if(task.equals("empty")){
						((TileEntityPoweredFluidCrafter) tile_two).tank.drain(((TileEntityPoweredFluidCrafter) tile_two).getTank().getFluid(), true);
					}
			...
			if(tile != null){
				if(tile instanceof TileEntityPoweredFluidCrafter){
					if(task.equals("empty")){
						((TileEntityPoweredFluidCrafter) tile).tank.drain(((TileEntityPoweredFluidCrafter) tile).getTank().getFluid(), true);
					}
					if(task.equals("mode_set")){
