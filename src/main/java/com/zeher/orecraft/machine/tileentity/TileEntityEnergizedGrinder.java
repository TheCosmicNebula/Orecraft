...
public class TileEntityEnergizedGrinder extends TileEntityLockable implements ITickable, ISidedInventory {
	private static final int[] SLOTS_TOP = new int[] { 0 };
	private static final int[] SLOTS_BOTTOM = new int[] { 2, 1 };
	private static final int[] SLOTS_SIDES = new int[] { 1 };
	private NonNullList<ItemStack> grinderItemStacks = NonNullList.<ItemStack>withSize(8, ItemStack.EMPTY);
	...
	public String getName() {
		return this.hasCustomName() ? this.grinderCustomName : "Energized Grinder";
	}
	...
	public static void registerFixesGrinder(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY,
				new ItemStackDataLists(TileEntityEnergizedGrinder.class, new String[] { "Items" }));
	}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		compound.setInteger("BurnTime", (short) this.grinderBurnTime);
	...
	public void update() {
		...
		if (!this.grinderItemStacks.get(5).isEmpty()) {
			smeltSpeed();
		} else {
			smeltSpeed();
		}
		if (!this.grinderItemStacks.get(6).isEmpty()) {
			powerSlot();
		} else {
			powerSlot();
		}
		...
		if (!this.world.isRemote) {
			...
			if(!this.isBurning() && this.hasPower() && this.canSmelt()){
				this.grinderBurnTime = cookSpeed;
			}
			if(this.isBurning()){
				flag1 = true;
			}
			...
			if (flag == this.isBurning()) {
				flag1 = true;
				BlockEnergizedGrinder.setState(!this.isBurning(), this.world, this.pos);
			}

			if (flag1) {
				this.markDirty();
			}
			if ((_power > _max_power) && this.grinderItemStacks.get(6).getCount() == 0) {
				_power = _max_power;
			}
			if ((_power > _max_power) && this.grinderItemStacks.get(6).getCount() == 1) {
				_power = 540000;
			}
			if ((_power > _max_power) && this.grinderItemStacks.get(6).getCount() == 2) {
				_power = 580000;
			}
			if ((_power > _max_power) && this.grinderItemStacks.get(6).getCount() == 3) {
				_power = 620000;
			}
			if ((_power > _max_power) && this.grinderItemStacks.get(6).getCount() == 4) {
				_power = 660000;
			}
		}
	}
	...
	public void smeltSpeed() {
		int i = this.grinderItemStacks.get(5).getCount();
		if (i == 0) {
			this.cookSpeed = 30;
		}
		if (i == 1) {
	...
	public void powerSlot() {
		int i = this.grinderItemStacks.get(6).getCount();
		if (i == 0) {
	...
	private boolean canSmelt() {
		if (((ItemStack) this.grinderItemStacks.get(0)).isEmpty()
				&& ((ItemStack) this.grinderItemStacks.get(2)).isEmpty()) {
			return false;
		}
		if (!(this.grinderItemStacks.get(0).isEmpty()) && (this.grinderItemStacks.get(2).isEmpty())) {
			ItemStack stack = MachineGrinderRecipes.instance().getGrindingResult(this.grinderItemStacks.get(0));
			if (stack.isEmpty()) {
				return false;
			} else {
				ItemStack stack1 = grinderItemStacks.get(4);
				if (stack1.isEmpty()) {
					return true;
				}
		...
		if (!(this.grinderItemStacks.get(2).isEmpty()) && (this.grinderItemStacks.get(0).isEmpty())) {
			ItemStack stack = MachineGrinderRecipes.instance().getGrindingResult(this.grinderItemStacks.get(2));
			if (stack.isEmpty()) {
				return false;
			} else {
				ItemStack stack1 = grinderItemStacks.get(3);
				if (stack1.isEmpty()) {
					return true;
				}
				if (!stack1.isItemEqual(stack)) {
			...
			ItemStack doubleStack0 = MachineGrinderRecipes.instance().getGrindingResult(this.grinderItemStacks.get(0));
			ItemStack doubleStack2 = MachineGrinderRecipes.instance().getGrindingResult(this.grinderItemStacks.get(2));

			if (doubleStack0.isEmpty() || doubleStack2.isEmpty()) {
				return false;
			} else {
				if (this.grinderItemStacks.get(3).isEmpty() || this.grinderItemStacks.get(4).isEmpty()) {
					return true;
				}
				if (!this.grinderItemStacks.get(3).isItemEqual(doubleStack2)
						|| !this.grinderItemStacks.get(4).isItemEqual(doubleStack0)) {
					return false;
				}
				int result = this.grinderItemStacks.get(3).getCount() + doubleStack2.getCount();
				int result2 = this.grinderItemStacks.get(4).getCount() + doubleStack0.getCount();
	...
	public void smeltItem() {
		if (this.canSmelt() && !(this.grinderItemStacks.get(0).isEmpty())
				&& !(this.grinderItemStacks.get(2).isEmpty())) {
			this._power -= TRZUtil.machinePowerDrain() * 2;
			ItemStack itemstack = (ItemStack) this.grinderItemStacks.get(2);
			ItemStack itemstack1 = MachineGrinderRecipes.instance().getGrindingResult(itemstack);
			ItemStack itemstack2 = (ItemStack) this.grinderItemStacks.get(0);
			ItemStack itemstack3 = MachineGrinderRecipes.instance().getGrindingResult(itemstack2);

			ItemStack outputStack1 = this.grinderItemStacks.get(3);
			ItemStack outputStack2 = this.grinderItemStacks.get(4);

			if (outputStack1.isEmpty()) {
				this.grinderItemStacks.set(3, itemstack1.copy());
				itemstack.shrink(1);
			...
			if (outputStack2.isEmpty()) {
				this.grinderItemStacks.set(4, itemstack3.copy());
				itemstack2.shrink(1);
		...
		if (this.canSmelt() && !(this.grinderItemStacks.get(0).isEmpty())
			...
			ItemStack outputStack2 = this.grinderItemStacks.get(4);
			if (outputStack2.isEmpty()) {
				this.grinderItemStacks.set(4, itemstack1.copy());
			} else if (outputStack2.getItem() == itemstack1.getItem()) {
				outputStack2.grow(itemstack1.getCount());
			}
			itemstack.shrink(1);
		}
		if (this.canSmelt() && !(this.grinderItemStacks.get(2).isEmpty())
				&& (this.grinderItemStacks.get(0).isEmpty())) {
			this._power -= TRZUtil.machinePowerDrain();
			ItemStack itemstack = this.grinderItemStacks.get(2);
			ItemStack itemstack1 = MachineGrinderRecipes.instance().getGrindingResult(itemstack);

			ItemStack outputStack2 = this.grinderItemStacks.get(3);
			if (outputStack2.isEmpty()) {
				this.grinderItemStacks.set(3, itemstack1.copy());
			} else if (outputStack2.getItem() == itemstack1.getItem()) {
				outputStack2.grow(itemstack1.getCount());
